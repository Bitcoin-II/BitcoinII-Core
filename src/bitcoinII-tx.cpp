...
static void SetupBitcoinIITxArgs(ArgsManager &argsman)
{
...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        std::string strUsage = CLIENT_NAME " bitcoinII-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.GetBoolArg("-version", false)) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "The bitcoinII-tx tool is used for creating and modifying bitcoinII transactions.\n\n"
                "bitcoinII-tx can be used with \"<hex-tx> [commands]\" to update a hex-encoded bitcoinII transaction, or with \"-create [commands]\" to create a hex-encoded bitcoinII transaction.\n"
                "\n"
                "Usage: bitcoinII-tx [options] <hex-tx> [commands]\n"
                "or:    bitcoinII-tx [options] -create [commands]\n"
                "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded bitcoinII transaction
