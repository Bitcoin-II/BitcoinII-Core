...
const std::string BitcoinIIGUI::DEFAULT_UIPLATFORM =
...
BitcoinIIGUI::BitcoinIIGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    if(enableWallet)
    {
        ...
        connect(walletFrame, &WalletFrame::createWalletButtonClicked, this, &BitcoinIIGUI::createWallet);
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &BitcoinIIGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &BitcoinIIGUI::showModalOverlay);
...
void BitcoinIIGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a BitcoinII address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and bitcoinII: URIs)"));
    ...
    connect(overviewAction, &QAction::triggered, this, &BitcoinIIGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &BitcoinIIGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &BitcoinIIGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your BitcoinII addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified BitcoinII addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed BitcoinII Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed BitcoinII Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a bitcoinII: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible BitcoinII command-line options").arg(CLIENT_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &BitcoinIIGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &BitcoinIIGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &BitcoinIIGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &BitcoinIIGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &BitcoinIIGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &BitcoinIIGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const auto& [path, info] : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &BitcoinIIGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &BitcoinIIGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, this, &BitcoinIIGUI::createWallet);
        ...
        connect(m_migrate_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const auto& [wallet_name, info] : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, wallet_name] {
                    ...
                    connect(activity, &MigrateWalletActivity::migrated, this, &BitcoinIIGUI::setCurrentWallet);
        ...
        connect(m_mask_values_action, &QAction::toggled, this, &BitcoinIIGUI::setPrivacy);
        connect(m_mask_values_action, &QAction::toggled, this, &BitcoinIIGUI::enableHistoryAction);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &BitcoinIIGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &BitcoinIIGUI::showDebugWindow);
}

void BitcoinIIGUI::createMenuBar()
{
    appMenuBar = menuBar();
...
void BitcoinIIGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &BitcoinIIGUI::setCurrentWalletBySelectorIndex);
...
void BitcoinIIGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &BitcoinIIGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &BitcoinIIGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &BitcoinIIGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &BitcoinIIGUI::showProgress);
...
#ifdef ENABLE_WALLET
void BitcoinIIGUI::enableHistoryAction(bool privacy)
{
...
void BitcoinIIGUI::setWalletController(WalletController* wallet_controller, bool show_loading_minimized)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &BitcoinIIGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &BitcoinIIGUI::removeWallet);
...
WalletController* BitcoinIIGUI::getWalletController()
{
    return m_wallet_controller;
}

void BitcoinIIGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &BitcoinIIGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &BitcoinIIGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &BitcoinIIGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &BitcoinIIGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &BitcoinIIGUI::incomingTransaction);
    connect(this, &BitcoinIIGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
...
void BitcoinIIGUI::removeWallet(WalletModel* walletModel)
{
...
void BitcoinIIGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame || !m_wallet_controller) return;
...
void BitcoinIIGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void BitcoinIIGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void BitcoinIIGUI::setWalletActionsEnabled(bool enabled)
{
...
void BitcoinIIGUI::createTrayIcon()
{
...
void BitcoinIIGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &BitcoinIIGUI::toggleHidden);
...
void BitcoinIIGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void BitcoinIIGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void BitcoinIIGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void BitcoinIIGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void BitcoinIIGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void BitcoinIIGUI::openClicked()
{
...
void BitcoinIIGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void BitcoinIIGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void BitcoinIIGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void BitcoinIIGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void BitcoinIIGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void BitcoinIIGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void BitcoinIIGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void BitcoinIIGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to BitcoinII network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void BitcoinIIGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void BitcoinIIGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void BitcoinIIGUI::updateHeadersSyncProgressLabel()
{
...
void BitcoinIIGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void BitcoinIIGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &BitcoinIIGUI::quitRequested);
...
void BitcoinIIGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void BitcoinIIGUI::createWallet()
{
#ifdef ENABLE_WALLET
#ifndef USE_SQLITE
    ...
    connect(activity, &CreateWalletActivity::created, this, &BitcoinIIGUI::setCurrentWallet);
    connect(activity, &CreateWalletActivity::created, rpcConsole, &RPCConsole::setCurrentWallet);
    activity->create();
#endif // ENABLE_WALLET
}

void BitcoinIIGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void BitcoinIIGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &BitcoinIIGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &BitcoinIIGUI::show);
                e->ignore();
            }
        }
    }
...
void BitcoinIIGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void BitcoinIIGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void BitcoinIIGUI::incomingTransaction(const QString& date, BitcoinIIUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(BitcoinIIUnits::formatWithUnit(unit, amount, true));
...
void BitcoinIIGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void BitcoinIIGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool BitcoinIIGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool BitcoinIIGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void BitcoinIIGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void BitcoinIIGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void BitcoinIIGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void BitcoinIIGUI::updateProxyIcon()
{
    std::string ip_port;
...
void BitcoinIIGUI::updateWindowTitle()
{
...
void BitcoinIIGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void BitcoinIIGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void BitcoinIIGUI::detectShutdown()
{
...
void BitcoinIIGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void BitcoinIIGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(BitcoinIIGUI* gui, const bilingual_str& message, const std::string& caption, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = BitcoinIIGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void BitcoinIIGUI::subscribeToCoreSignals()
{
...
void BitcoinIIGUI::unsubscribeFromCoreSignals()
{
...
bool BitcoinIIGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<BitcoinIIUnit> units = BitcoinIIUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const BitcoinIIUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, BitcoinIIUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const BitcoinIIUnit u : BitcoinIIUnits::availableUnits()) {
        menu->addAction(BitcoinIIUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(BitcoinIIUnit newUnits)
{
    setText(BitcoinIIUnits::longName(newUnits));
}
