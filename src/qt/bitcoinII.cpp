...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. bitcoinII_de.qm (shortcut "de" needs to be defined in bitcoinII.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. bitcoinII_de_DE.qm (shortcut "de_DE" needs to be defined in bitcoinII.qrc)
...
static const char* qt_argv = "bitcoinII-qt";

BitcoinIIApplication::BitcoinIIApplication()
...
void BitcoinIIApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the BitcoinIIApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", BitcoinIIGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void BitcoinIIApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool BitcoinIIApplication::createOptionsModel(bool resetSettings)
{
...
void BitcoinIIApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new BitcoinIIGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &BitcoinIIGUI::quitRequested, this, &BitcoinIIApplication::requestShutdown);
...
void BitcoinIIApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void BitcoinIIApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool BitcoinIIApplication::baseInitialize()
{
    return node().baseInitialize();
}

void BitcoinIIApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &BitcoinIIApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &BitcoinIIApplication::handleRunawayException);
    connect(this, &BitcoinIIApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &BitcoinIIApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void BitcoinIIApplication::parameterSetup()
{
...
void BitcoinIIApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void BitcoinIIApplication::requestInitialize()
{
...
void BitcoinIIApplication::requestShutdown()
{
...
void BitcoinIIApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
...
void BitcoinIIApplication::handleNonFatalException(const QString& message)
{
...
WId BitcoinIIApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool BitcoinIIApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", BitcoinIIGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    for (int i = 1; i < argc; i++) {
...
#ifdef ENABLE_WALLET
        if (arg.startsWith(BITCOINII_IPC_PREFIX, Qt::CaseInsensitive)) {
            invalid_token &= false;
        ...
        if (invalid_token) {
            InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see bitcoinII-qt -h for a list of options.", argv[i])));
            QMessageBox::critical(nullptr, CLIENT_NAME,
                                  // message cannot be translated because translations have not been initialized
                                  QString::fromStdString("Command line contains unexpected token '%1', see bitcoinII-qt -h for a list of options.").arg(QString::fromStdString(argv[i])));
            return EXIT_FAILURE;
        }
    }
    ...
    /// 6-7. Parse bitcoinII.conf, determine network, switch to network specific
    ...
    // bitcoinII: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
